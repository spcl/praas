...
namespace praas::control_plane::backend {
  ...
  //    if (!ptr)
  //      spdlog::error(
  //          "Couldn't connect to a local process server at {}",
  //          options.local_server
  //      );
  //  } else {
  //    // FIXME
  //  }
  ...
  //  spdlog::debug(
  //      "Connect control plane to local worker at {}:{}.", ip_address, port
  //  );
  //  try {
  ...
  //      max_sessions, controller_port, controller_ip_address, process_id
  //  );
  //  // FIXME: handle errors here
  //  connection.write(req.data.data(), size);
  //  spdlog::debug(
  //      "Allocating a new process {} of name {} with max sessions {}, process "
  //      "will talk to {}:{}",
  //      process_id, process_name, max_sessions, controller_ip_address,
  //      controller_port
  //  );
  //}

  //void AWSBackend::allocate_process(std::string, std::string, int16_t)
  //{
  //  // FIXME
  //}
