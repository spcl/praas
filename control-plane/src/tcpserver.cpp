...
namespace praas::control_plane::tcpserver {
  ...
  )
  {
    if (buffer->readableBytes() < praas::common::message::Message::BUF_SIZE) {
      buffer->read(praas::common::message::Message::BUF_SIZE - buffer->readableBytes());
    }
    ...
    if (std::holds_alternative<common::message::ProcessConnectionParsed>(msg)) {
      auto& conn_msg = std::get<common::message::ProcessConnectionParsed>(msg);
      ...
      auto it = _pending_processes.find(std::string{conn_msg.process_name()});
      if (it != _pending_processes.end()) {
        ...
        _pending_processes.erase(it);
        {
          ...
          process_ptr->connect(connectionPtr);
        }
        // Store process reference for future messages
        connectionPtr->setContext(std::move(process_ptr));
