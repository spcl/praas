...
namespace praas::control_plane {
  ...
  )
  {
    ...
    if (!session_id.empty()) {
      ...
      if (redis_conn.sismember("SESSIONS_ALIVE_" + process_id, session_id)) {
        ...
        std::from_chars(
            func.data(), func.data() + func.length(), max_functions
        );
    ...
    if (proc) {
      ...
      ssize_t size =
          req.fill(session_id, session.max_functions, session.memory_size);
      ...
      //  spdlog::error(
      //    "Incorrect allocation of sesssion {} at {}, received {} bytes, error
      //    code {}", session_id, proc->connection.peer_address().to_string(),
      //    bytes, ret
      //  );
      //  // FIXME: notify user
      //} else {
      //  spdlog::debug("Succesful allocation of sesssion {} at {}", session_id,
      //  proc->connection.peer_address().to_string()); session.allocated =
      //  true;
      //}
    ...
    // New process
      ...
      backend.allocate_process(
          process_name.value(), id, max_sessions_per_process
      );
    }
  ...
  )
  {
...

    {
      ...
      for (Session* session : proc->sessions) {
        if (!session->allocated) {
          std::string session_id = session->session_id;
          ssize_t size = req.fill(
              session_id, session->max_functions, session->memory_size
          );
          // Send allocation request
          proc->connection.write(req.data, size);
          ...
          // if(bytes != sizeof(ret) || ret) {
          //   spdlog::error(
          //     "Incorrect allocation of sesssion {}, received {} bytes, error
          //     code {}", session_id, bytes, ret
          //   );
          //   // FIXME: notify user
          // } else {
    ...
    if (!server.add_epoll(
            proc->connection.handle(), proc,
            EPOLLIN | EPOLLPRI | EPOLLRDHUP | EPOLLONESHOT
        )) {
  ...
  )
  {
    ...
    if (!allocations.size()) {
      ...
      while (!allocations.empty()) {
        ...
        spdlog::debug(
            "Invoking function {}, invocation id {}, on session {}, sending {} "
            "bytes.",
  ...
  )
  {
    ...
    redis_conn.sadd(
        "SESSIONS_SWAPPED_" + process->global_process_id, session_id
    );
  ...
  )
  {
    ...
    if (recv_data == 0) {
      spdlog::info(
          "End of file on connection with {}.", conn.peer_address().to_string()
      );
    ...
    // FIXME: handle timeout & interruption
      ...
      spdlog::error(
          ...
          conn.peer_address().to_string(), strerror(errno)
      );
      worker.handle_process_closure(process);
      return;
    }
    ...
    if (!msg) {
      spdlog::error(
          "Incorrect message of size {} received from {}.", recv_data,
    ...
    if (msg.get()->type() == praas::common::MessageType::Type::SESSION_STATUS) {
      auto casted_msg =
          dynamic_cast<praas::common::SessionStatusMessage*>(msg.get());
      // closure
      // FIXME: encapsulate
      if (casted_msg->session_status() == -1)
        worker.handle_session_closure(
            process, casted_msg->memory_size(), casted_msg->session_id()
        );
      else
        spdlog::debug(
            "Session {} allocation status {}", casted_msg->session_id(),
            casted_msg->session_status()
        );
    } else {
    ...
    if (!worker.server.mod_epoll(
            process->connection.handle(), process,
            EPOLLIN | EPOLLPRI | EPOLLRDHUP | EPOLLONESHOT
        )) {
      spdlog::error("Couldn't add the process to epoll,closing connection");
  ...
  void Worker::worker(sockpp::tcp_socket* conn)
  {
    ...
    // Correctly received request
      ...
      if (msg.get()->type() == praas::common::MessageType::Type::CLIENT) {
        ...
        worker.process_process(
            conn, dynamic_cast<praas::common::ProcessMessage*>(msg.get())
        );
      } else if (msg.get()->type() == praas::common::MessageType::Type::SESSION) {
        worker.process_session(
            conn, dynamic_cast<praas::common::SessionMessage*>(msg.get())
        );
      } else {
