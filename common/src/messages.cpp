...
namespace praas::common {
  ...
  std::unique_ptr<MessageType> Header::parse()
  {
    ...
    if (type == static_cast<int16_t>(MessageType::Type::CLIENT)) {
      return std::make_unique<ClientMessage>(data.data() + 2);
    } else if (type == static_cast<int16_t>(MessageType::Type::PROCESS)) {
      return std::make_unique<ProcessMessage>(data.data() + 2);
    } else if (type == static_cast<int16_t>(MessageType::Type::SESSION)) {
      return std::make_unique<SessionMessage>(data.data() + 2);
    } else if (type == static_cast<int16_t>(MessageType::Type::SESSION_STATUS)) {
      return std::make_unique<SessionStatusMessage>(data.data() + 2);
    } else {
      return nullptr;
    }
  }
