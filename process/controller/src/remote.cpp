...
namespace praas::process::remote {
  ...
  TCPServer::TCPServer(Controller& controller, const config::Controller& cfg)
      : _is_running(true), _controller(controller),
        _server(_loop_thread.getLoop(), trantor::InetAddress(cfg.port), "tcpserver")
  {
    ...
    _server.setConnectionCallback([this](const trantor::TcpConnectionPtr& connectionPtr) {
      if (connectionPtr->connected()) {
        SPDLOG_LOGGER_DEBUG(
            _logger, "New connection from {}", connectionPtr->peerAddr().toIpPort()
        );
    ...
    _loop_thread.getLoop()->runOnQuit({[this]() {
      ...
      if (eptr) {
        try {
          ...
          _logger->error("Exception thrown {}", e.what());
        } catch (const std::string& e) {
          _logger->error("Exception thrown {}", e);
        } catch (const char* e) {
          _logger->error("Exception thrown {}", e);
        } catch (...) {
          _logger->error("Exception thrown, unknown!");
        }
      }
    }});
  ...
  void TCPServer::poll(std::optional<std::string> control_plane_address)
  {
    ...
    auto conn = std::make_shared<Connection>(
        Connection::Status::CONNECTING, RemoteType::CONTROL_PLANE, std::nullopt, nullptr
    );
    ...
    (*iter).second->client = std::make_shared<trantor::TcpClient>(
        this->_server.getLoop(), trantor::InetAddress{address, static_cast<uint16_t>(port)},
        "client"
    );
   ...
   *
   **/
  void TCPServer::_handle_message(
  ...
  )
  {
    ...
    SPDLOG_LOGGER_DEBUG(
        _logger, "Received {} bytes active connection? {}", buffer->readableBytes(), conn != nullptr
    );
    ...
    SPDLOG_LOGGER_DEBUG(
        _logger, "Consumed message? {} There are {} bytes remaining", consumed,
  ...
  )
  {
    ...
    if (buffer->readableBytes() >= msg.payload_size()) {
      ...
      SPDLOG_LOGGER_DEBUG(
          _logger, "Received complete invocation request of {}, with {} bytes of input",
          msg.function_name(), msg.payload_size()
      );
      ...
      if (connection.type == RemoteType::DATA_PLANE) {
        ...
        _controller.remote_message(
            std::move(connection.cur_msg), std::move(buf), connection.id.value()
        );
      }
  ...
  )
  {
    ...
    if (buffer->readableBytes() >= connection.bytes_to_read) {
      ...
      SPDLOG_LOGGER_DEBUG(
          _logger, "Received invocation result for id {}, with {} bytes of input",
          msg.invocation_id(), msg.total_length()
      );
  ...
  )
  {
    ...
    if (buffer->readableBytes() >= connection.bytes_to_read) {
      ...
      SPDLOG_LOGGER_DEBUG(
          _logger, "Finished processing PUT, {} remaining bytes", buffer->readableBytes()
      );
      return true;
    }
    // Not enough payload, not consumed
      ...
      if (find_iter != _connection_data.end()) {
        ...
        if (msg.process_name() == DATAPLANE_ID) {
          conn = std::make_shared<Connection>(
              Connection::Status::CONNECTED, RemoteType::DATA_PLANE, std::nullopt, connectionPtr
          );
          _data_plane = conn;
        } else if (msg.process_name() == CONTROLPLANE_ID) {
          conn = std::make_shared<Connection>(
              Connection::Status::CONNECTED, RemoteType::CONTROL_PLANE, std::nullopt, connectionPtr
          );
          _control_plane = conn;
        } else {
          conn = std::make_shared<Connection>(
              Connection::Status::CONNECTED, RemoteType::PROCESS, std::string{msg.process_name()},
              connectionPtr
          );
        }
  ...
  )
  {
    _controller.update_application(
        static_cast<common::Application::Status>(msg.status_change()), msg.process_id()
    );
  ...
  void TCPServer::put_message(
      std::string_view process_id, std::string_view name, runtime::internal::Buffer<char>&& payload
  )
  {
    std::unique_lock<std::mutex> lock{_conn_mutex};
  ...
  void TCPServer::_connect(Connection* conn)
  {
    ...
    conn->client = std::make_shared<trantor::TcpClient>(
        ...
        trantor::InetAddress{conn->ip_address, static_cast<uint16_t>(conn->port)}, "client"
    );
    ...
    conn->client->setConnectionCallback(
        [this, connection = conn](const trantor::TcpConnectionPtr& conn) -> void {
          if (conn->connected()) {
            ...
            SPDLOG_LOGGER_DEBUG(
                _logger, "Process connection between {} and {} disconnected",
                conn->localAddr().toIpPort(), conn->peerAddr().toIpPort()
            );
          }
        }
    );
    ...
    conn->client->setMessageCallback(
        [this,
         connection =
             conn](const trantor::TcpConnectionPtr& conn, trantor::MsgBuffer* buffer) -> void {
          SPDLOG_LOGGER_DEBUG(
              _logger, "Callback from the client connection! {} bytes to read",
              buffer->readableBytes()
          );
          _handle_message(conn, buffer);
        }
