...
extern "C" int aws_sdk_init(praas::function::Invocation invocation, praas::function::Context& context)
{
  ...
  clientConfiguration = new Aws::S3::S3ClientConfiguration{};
  clientConfiguration->executor = Aws::MakeShared<Aws::Utils::Threading::PooledThreadExecutor>("exec", 32);
  std::cerr << "ok" << std::endl;
  client = new Aws::S3::S3Client(*clientConfiguration);
  return 0;
}

extern "C" int s3_swapper(praas::function::Invocation invocation, praas::function::Context& context)
{
  Invocations in;
  ...
  std::vector<std::promise<void>> waits;
  waits.resize(in.threads);
  ...
  for(int size : in.sizes) {
    ...
    for(int i = 0; i  < in.repetitions + 1; ++i) {
      ...
      for(int j = 0; j < in.threads; ++j) {

        waits[j] = std::promise<void>{};

      }

      //std::string msg_key = fmt::format("send_{}_{}", size, i);
      auto begin = std::chrono::high_resolution_clock::now();
      int size_per_thread = size / in.threads;
      for(int j = 0; j < in.threads; ++j) {

        auto key = fmt::format("swap/{}_{}", size, j);
        Aws::S3::Model::PutObjectRequest request;
        request.SetBucket(in.bucket);
        request.SetKey(key);

        const std::shared_ptr<Aws::IOStream> input_data
        = std::make_shared<boost::interprocess::bufferstream>(state.data() + j *size_per_thread, size_per_thread);
        request.SetBody(input_data);
        std::cerr << fmt::format("thread {} stores {} starting at {}", j, size_per_thread, j*size_per_thread) << std::endl;
        ...
        client->PutObjectAsync(request,
          [&waits, j](const Aws::S3::S3Client *s3Client,
...
extern "C" int s3_sync_swapper(praas::function::Invocation invocation, praas::function::Context& context)
{
  Invocations in;
  invocation.args[0].deserialize(in);
  //std::cerr << "Start benchmark, input size " << invocation.args[0].len << " data size" << in.data.size() << std::endl;
