...
int main(int argc, char** argv)
{
  ...
  std::optional<std::string> process_id = std::nullopt;
  ...
  for (std::string file : cfg.input_files) {
    ...
    for (int i = 0; i < cfg.repetitions + 1; ++i) {
      ...
      auto invoc = praas.invoke("test-latex-editing", "update-file", data, process_id, cfg.resources_cpu, cfg.resources_memory);
      auto end = std::chrono::high_resolution_clock::now();
      if (invoc.return_code != 0) {
        std::cerr << invoc.return_code << " " << invoc.error_message << " " << file << " " << i << std::endl;
        abort();
      }
      if (i > 0) {
        measurements.emplace_back(
            "update-file", file, i, data.length(), invoc.response.length(),
            std::chrono::duration_cast<std::chrono::nanoseconds>(end - begin).count()
        );
      }
      {
        std::ofstream output{fmt::format("output_update_file_{}_praas_{}", j, i)};
        output << invoc.response;
      }

      if(!process_id.has_value())
	      process_id = invoc.process_name;
    }
  ...
  for (std::string file : cfg.input_files) {
    ...
    for (int i = 0; i < cfg.repetitions + 1; ++i) {
      ...
      auto begin = std::chrono::high_resolution_clock::now();
      auto invoc = praas.invoke("test-latex-editing", "get-file", data, process_id, cfg.resources_cpu, cfg.resources_memory);
      auto end = std::chrono::high_resolution_clock::now();
      if (invoc.return_code != 0) {
      ...
      if (i > 0) {
        measurements.emplace_back(
            "get-file", file, i, data.length(), invoc.response.length(),
	    std::chrono::duration_cast<std::chrono::nanoseconds>(end - begin).count()
        );
      }
      {
